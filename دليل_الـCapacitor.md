...
## ⚠️ مهم جداً: PWABuilder لا يصلح لتوفير — استخدم مسار Capacitor حصراً

إذا حوّلت المنصة إلى APK عبر **PWABuilder** ستفشل الأذونات الأصلية حتماً:

| المشكلة في APK الـPWABuilder | السبب |
|---|---|
| زر الموقع يقول «لا توجد صلاحيات» ولا تظهر نافذة السماح | APK الـPWABuilder لا يحوي `ACCESS_FINE_LOCATION`/`ACCESS_COARSE_LOCATION` في الـManifest — أندرويد يرفض الطلب صامتاً |
| زر الإشعارات في إعدادات أندرويد **معطّل (رمادي)** لا يمكن تفعيله | لا يوجد `POST_NOTIFICATIONS` في الـManifest ولا قناة إشعارات FCM أصلية |
| لا تصل إشعارات النداء للمناديب | لا يوجد `TawfirFirebaseMessagingService` أصلي — WebView بلا PushManager لا يستقبل FCM |

**الحل الوحيد:** ابنِ الـAPK عبر مسار Capacitor الموثّق في هذا الدليل (GitHub Actions `build-android.yml` أو المحلي) — سكربت `patch-android-identity.mjs` يضيف تلقائياً:
- كل أذونات الـManifest (موقع دقيق + تقريبي + إشعارات + اهتزاز + شبكة)
- طلب إذن الإشعارات وقت التشغيل (نافذة السماح الأصلية عند أول فتح)
- إضافة `TawfirNative` بنوافذ السماح الأصلية للموقع والإشعارات من داخل الـWebView + زر «افتح إعدادات التطبيق» للتعافي بعد الرفض
- `@capacitor/geolocation` لنافذة سماح الموقع الحقيقية + GPS دقيق
- خدمة FCM الأصلية (إشعارات النداء للمناديب + إشعارات الطلبات للعملاء)
